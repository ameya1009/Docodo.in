...
function SignupForm() {
  ...
  const [error, setError] = useState(
    urlError === "OAuthSignin" || urlError === "OAuthCallback" || urlError === "OAuthCreateAccount"
      ? "Google signup failed. Please verify your Google account or create an account with email."
      : urlError
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      setError("Failed to initialize Google Signup. Please use email signup.");
    ...
    <div className="w-full max-w-md relative z-10">
      ...
      <div className="glass rounded-2xl p-8 border border-[var(--border-default)]">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white text-gray-800 font-semibold rounded-xl hover:bg-gray-50 transition-colors mb-6 text-sm disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
        >
          {googleLoading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={isPending || googleLoading} className="w-full py-3.5 bg-[var(--lime)] text-[var(--bg-void)] font-bold rounded-xl hover:bg-[var(--lime-hover)] transition-colors flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed mt-2 shadow-[var(--lime-glow-md)]">
